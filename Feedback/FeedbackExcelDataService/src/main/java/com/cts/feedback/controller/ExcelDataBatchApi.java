...
@CrossOrigin("http://localhost:3000") //connection to Reactjs to Spring Boot
public interface ExcelDataBatchApi {

	Logger log = LoggerFactory.getLogger(ExcelDataBatchApi.class);
	...
	@RequestMapping(value = "/importEvents", produces = { "application/json" }, method = RequestMethod.GET)
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "OK", response = EventReport.class),
			@ApiResponse(code = 201, message = "Created"), @ApiResponse(code = 401, message = "Unauthorized"),
			@ApiResponse(code = 403, message = "Forbidden"), @ApiResponse(code = 404, message = "Not Found") })
	@RequestMapping(value = "/importSummary", produces = { "application/json" }, method = RequestMethod.GET)
		...
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
			if (getAcceptHeader().get().contains("application/json")) {
				try {
					...
					log.error("Could not serialize response for content type application/json", e);
					return Flux.just(new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR));
				}
			}
		} else {
